#include "MeOrion.h"
   ...
   * VARIABLES
   */
    uint8_t pins[6]; // sensor information - for more complex line correction
    String pinString; // for simple case - turns, forward
  ...
  void getBin(uint8_t dec) {
    ...
    // put into array (have to account for 0s in the front
    while (bin > -1) {
      if (binaryLength > 0) {
        pins[bin] = binary.charAt(binaryLength - 1) - 48;
        binaryLength--;
      } else { // account for front zeros
        pins[bin] = 0;
      }
      bin--;
    }
